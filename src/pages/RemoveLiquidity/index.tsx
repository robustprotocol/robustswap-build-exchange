...
export default function RemoveLiquidity({
  ...
  const onRemove = async () => {
    ...
    const amountsMin = {
      [Field.CURRENCY_A]: calculateSlippageAmount(currencyAmountA, 50)[0],
      [Field.CURRENCY_B]: calculateSlippageAmount(currencyAmountB, 50)[0],
    }
  ...
  const modalHeader = () => {
    return (
      <AutoColumn gap="md" style={{ marginTop: '20px' }}>
        ...
        <Italic fontSize={12} color="white" textAlign="left" padding="12px 0 0 0">
  ...
  const modalBottom = () => {
    return (
      <>
        <RowBetween>
          <Text color="white">{`FLIP ${currencyA?.symbol}/${currencyB?.symbol}`} Burned</Text>
          <RowFixed>
  ...
  return (
    <>
      <AppBody>
        <AddRemoveTabs adding={false} currencyA={currencyA?.symbol} currencyB={currencyB?.symbol} />
        <Wrapper>
      ...
      {pair ? (
        <AutoColumn style={{ maxWidth: '417px', width: '90%', marginTop: '1rem' }}>
